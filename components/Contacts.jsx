...
const Contacts = () => {
  ...
  return (
    ...
    <form id='form' action="/api/form" method='POST' className={`flex flex-col justify-center items-start px-5 py-10 w-full duration-500 ease-in-out ${success == true ? 'hidden' : ''} overflow-hidden`} onSubmit={onSub}>
        ...
        <input type="text" id='name' name='name' title='Please enter your name' minLength={2} maxLength={20} required value={name} onChange={(e)=> setName(values.name = e.target.value)} className='md:text-base text-sm px-5 md:max-w-[40%] w-[60%] bg-slate-900 border-slate-600 border rounded-[5px] shadow-inner shadow-black placeholder-slate-600 focus:outline-cyan-600'/>
        <br />
        <label htmlFor="email" className='md:text-base text-sm text-slate-500 font-bold'>Email:</label>
        <input type="email" id='email' name='email' required value={email} onChange={(e)=> setEmail(values.email = e.target.value)} className='md:text-base text-sm px-5 md:max-w-[40%] w-[60%] bg-slate-900 border-slate-600 border rounded-[5px] shadow-inner shadow-black placeholder-slate-600 focus:outline-cyan-600'/>
        <br />
        ...
        <textarea name="message" id="message" cols="30" rows="10" required value={message} onChange={(e)=> setMessage(values.message = e.target.value)} className='md:text-base text-sm  md:w-[60%] w-full px-5 py-2 bg-slate-900 border border-slate-600 rounded-[10px] shadow-inner shadow-black placeholder-slate-600 focus:outline-cyan-600' />
        <br />  
